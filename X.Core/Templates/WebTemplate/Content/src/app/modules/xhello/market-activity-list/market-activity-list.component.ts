...
import { MarketActivityService } from '../market-activity.service';

@Component({
  selector: 'market-activity-list',
  templateUrl: './market-activity-list.component.html',
  styleUrls: ['./market-activity-list.component.scss']
})
export class MarketActivityListComponent implements OnInit, AfterViewInit {
      ...
      constructor(public mainService: MarketActivityService,
      ...
      async ngOnInit() {
          this.pageTitleService.setTitle('Views.' + 'MarketActivityList');
              await this.init();
      }
      ...
      details(row) {
          this.router.navigate(['/marketActivties', { id: row.id }]);
      }
