...
namespace X.Core.Generator
{
    public class StateMachineModelGenerator : ICodeGenerator
    {
        ...
        protected virtual async Task<SyntaxList<MemberDeclarationSyntax>> GenerateTriggersAndStates(TransformationContext context)
        {
            ...
            stateMachine.Parse(_model, applyToClass.Identifier.ValueText);
            ...
            int triggerId = 0;
            foreach (var trigger in stateMachine.TriggerDefinitions)
            {
                triggerEnum = triggerEnum.AddMembers(SyntaxFactory.EnumMemberDeclaration(
                        SyntaxFactory.Identifier(trigger.TriggerName))
                    .WithEqualsValue(
                        SyntaxFactory.EqualsValueClause(
                            SyntaxFactory.LiteralExpression(
                                SyntaxKind.NumericLiteralExpression,
                ...
                var classDeclaration = SyntaxFactory.ClassDeclaration($"{applyToClass.Identifier.ValueText}{trigger.TriggerName}Request");
                ...
                results = results.Add(classDeclaration);
            }
